        ...
        const vector<int> & n_t) -> void
{
    ...
    *_imp->proof_stream << "j " << _imp->proof_line << " 1 ~x" << _imp->variable_mappings[pair{ p.first, t.first }] << " >= 1 ;" << "\n";
    ++_imp->proof_line;
    _imp->eliminations.emplace(pair{ p.first, t.first }, _imp->proof_line);
        ...
        ) -> void
{
    ...
    *_imp->proof_stream << " >= 1 ;" << "\n";
    ...
    // if p maps to t then q does not map to t
    *_imp->proof_stream << "p " << _imp->proof_line << " " << _imp->injectivity_constraints[t.first] << " + 0" << "\n";
    ++_imp->proof_line;
